...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    ...
    if (storedCart && storedAddress) {
      ...
      // Send messages to relevant sellers and update their order counts
      Object.entries(sellerItems).forEach(([sellerId, items]) => {
        const sellerProducts = items.map(item => ({
          productName: item.name,
          quantity: item.quantity
        ...
        const sellerOrders = JSON.parse(localStorage.getItem(`sellerOrders_${sellerId}`) || '[]')
        sellerOrders.push({
        ...
        localStorage.setItem(`sellerOrders_${sellerId}`, JSON.stringify(sellerOrders))
      })
    }
